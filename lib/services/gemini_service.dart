  ...
  Future<List<Meal>> generateMealsFromIngredients(String ingredients) async {
    try {
      ...
      if (response.body.length > 500) {
        print('GenAI: Response body starts with: ${response.body.substring(0, 500)}...');
      } else {
      ...
      if (response.statusCode == 200) {
        try {
          ...
          throw Exception('Failed to understand server response. Format error.');
        }
      } else {
      ...
      if (e is http.ClientException || e.toString().contains('SocketException')) {
        throw Exception('Network error. Ensure your phone is on the same Wi-Fi as your computer (IP: 192.168.1.76) and port 3500 is open in your firewall.');
      } else if (e.toString().contains('TimeoutException')) {
        throw Exception('The Chef is taking too long (over 2 mins). Please try a simpler request.');
      }
